...
class DinnerView: BaseUIView {
    ...
    private var menuData: [ChangeMenuTableResponse] = [] {
        didSet {
//            dataBindMenu(menuData: menuData)
            [dormitoryTableView, dodamTableView, studentTableView].forEach { $0.reloadData() }
        }
    }
...
//            cell.configureData(changleDummy[indexPath.row])
            cell.bind(menuData: menuData[indexPath.row])
        } else {
            cell.configureCell(fixedDummy[indexPath.row])
        }
        return cell
    }
        ...
        self.menuProvider.request(.getChangeMenuTableResponse(date: date, restaurant: restaurant, time: time)) { response in
            ...
            case .success(let moyaResponse):
                do {
                    ...
                    self.menuData = [responseDetailDto]
